...
import { BookListQueryModel } from '@/types/book'
...
interface FilterProps {
  ...
  onSelect: (opt: ValueOf<Pick<BookListQueryModel, 'order' | 'filter'>>) => void
}

export const Filter = ({ id, name, placeholder, selectedOption, options, onSelect }: FilterProps) => {
  const containerRef = useRef<HTMLDivElement>(null)

  const { isOpen: isDialogOpen, dialogRef: selectBoxRef, handleToggleDialog, handleDialogClose } = useDialog()

  const UUID = `filter-${id}`

  const handleClickOption = (option: string) => () => {
    onSelect(option as ValueOf<Pick<BookListQueryModel, 'order' | 'filter'>>)
    handleDialogClose()
  }
